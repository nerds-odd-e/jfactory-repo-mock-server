...
功能: Get/Post/Put request with Chinese steps
  ...
  Rule: Response by url and params
    ...
    场景: GET by url and params without setting data with table
      假如存在1个接口数据"Bean"并匹配查询参数"foo=bar&name=value1&name=value2"
      那么"/beans" should response:
      """
      code= 404
      """
    ...
    场景大纲: <method> by url and params without setting data with table
      假如存在1个接口数据"<factory>"并匹配查询参数"foo=bar&name=value1&name=value2"
      当<method> "/beans":
      """
      {}
      """
  ...
  Rule: Respones by url and path variables

    场景: GET by url and path variables without setting data with table
      假如存在1个接口数据"BeanWithPathVariable"并匹配路径变量"foo=bar"
      那么"/beans" should response:
      """
        code= 404
      """
      那么"/beans/bar" should response:
      """
      : {
      code: 200
        body.json= {
    ...
    场景大纲: <method> by url and path variables without setting data with table
      假如存在1个接口数据"<factory>"并匹配路径变量"foo=bar"
      当<method> "/beans":
      """
      {}
      """
